...
export function FremixCuratedVideos(){
  ...
  return createPortal(
    <section id="amostras-fremix" style={s.dark}>
      <style>{`.fm-curated-card{transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}.fm-curated-card:hover{transform:translateY(-3px);border-color:rgba(244,114,182,.55)!important;box-shadow:0 16px 40px rgba(0,0,0,.24)}.fm-curated-card:focus-visible{outline:3px solid #f472b6;outline-offset:3px}@media(max-width:900px){.fm-curated-layout{grid-template-columns:1fr!important}.fm-curated-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important}}@media(max-width:560px){.fm-curated-grid{grid-template-columns:1fr!important}.fm-curated-shell{padding-left:16px!important;padding-right:16px!important}.fm-curated-player{min-height:230px!important}.fm-curated-heading{align-items:flex-start!important;flex-direction:column!important}}`}</style>
...
const s:Record<string,React.CSSProperties>={
  dark:{background:"linear-gradient(180deg,#100b16,#17101f 60%,#110c17)",color:"white"},
  ...
  eyebrow:{display:"inline-flex",alignItems:"center",gap:6,fontSize:10,fontWeight:950,letterSpacing:".14em",color:"#f0b6d3"},
  ...
  lead:{maxWidth:780,color:"#b7aabd",fontSize:13,lineHeight:1.7,margin:0},
  ...
  player:{aspectRatio:"16/9",minHeight:360,borderRadius:19,overflow:"hidden",background:"#050407",border:"1px solid rgba(255,255,255,.1)",boxShadow:"0 30px 75px rgba(0,0,0,.33)"},
  iframe:{width:"100%",height:"100%",border:0,display:"block"},
  playerMeta:{display:"flex",justifyContent:"space-between",gap:14,alignItems:"center",padding:"12px 3px",color:"#aa9daf",fontSize:11},
  ...
  cardActive:{borderColor:"#f472b6",background:"rgba(244,114,182,.09)"},
  thumbWrap:{position:"relative",aspectRatio:"16/9",overflow:"hidden",background:"#08060a"},
  ...
  number:{position:"absolute",left:7,top:7,padding:"4px 6px",borderRadius:6,background:"rgba(9,6,12,.75)",color:"#f1d9e6",fontWeight:900,fontSize:9},
  cardText:{display:"block",padding:"9px 10px",fontSize:10,fontWeight:800,color:"#d6cbd9"},
  note:{display:"flex",gap:8,flexWrap:"wrap",marginTop:18,paddingTop:16,borderTop:"1px solid rgba(255,255,255,.09)",fontSize:10,color:"#948899"},
};