...
import { CheckCircle2, Circle, Clock3, MapPin, PackageCheck, ShoppingBag, Truck } from "lucide-react";
import { supabase } from "../lib/roles";
import type { MerchantOrder, OrderStatus } from "../lib/merchantPlatform";
...
export function CustomerOrders() {
  ...
  async function load() {
    ...
    const rows = (data ?? []).map((row: any) => ({ ...row, subtotal:Number(row.subtotal||0), delivery_fee:Number(row.delivery_fee||0), discount:Number(row.discount||0), platform_fee:Number(row.platform_fee||0), total:Number(row.total||0), items:row.order_items ?? [] })) as MerchantOrder[];
    setOrders(rows);
  ...
  return <main style={s.page}>
    ...
    <div style={s.layout}>
      ...
      {order && <section style={s.detail}>
        <div style={s.detailTop}><div><span style={s.kicker}>PEDIDO #{order.order_number}</span><h2 style={s.h2}>{order.status === "cancelled" ? "Pedido cancelado" : order.status === "delivered" ? "Pedido entregue" : "Seu pedido está em andamento"}</h2></div><strong style={s.total}>{brl.format(order.total)}</strong></div>
