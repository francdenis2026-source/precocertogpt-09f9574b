...
export function DorinhaCommerceEnhancer(){
 ...
 return <div style={s.overlay} role="dialog" aria-modal="true" aria-labelledby="pix-title"><style>{`
  .pc-pix-modal h2{font-size:clamp(1.3rem,2.5vw,1.65rem);line-height:1.05;margin:3px 0}
  .pc-pix-modal h3,.pc-pix-modal p{margin:2px 0}
  .pc-pix-head-art{position:absolute;right:48px;top:50%;width:132px;height:78px;transform:translateY(-50%);filter:drop-shadow(0 9px 15px rgba(50,26,57,.14))}
  .pc-pix-modal input:focus,.pc-pix-modal select:focus{border-color:#79547f!important;box-shadow:0 0 0 3px rgba(121,84,127,.10)}
  @media(max-width:620px){.pc-pix-fields{grid-template-columns:1fr!important}.pc-pix-modal{max-height:calc(100dvh - 8px)!important;border-radius:13px!important}.pc-pix-form{padding:10px 14px 12px!important}.pc-pix-summary{grid-template-columns:1fr auto!important}.pc-pix-modal .pc-pix-summary>div:last-child{text-align:right!important}.pc-pix-head-art{display:none}.pc-pix-head{padding-right:48px!important}}
  @media(max-height:720px) and (min-width:621px){.pc-pix-modal .pc-pix-fields{gap:5px 9px!important}.pc-pix-modal input,.pc-pix-modal select{min-height:34px!important}.pc-pix-modal .pc-pix-form{gap:5px!important;padding-top:8px!important;padding-bottom:9px!important}.pc-pix-head{padding-top:9px!important;padding-bottom:8px!important}}
 `}</style><div className="pc-pix-modal" style={s.modal}>
  ...
  <div style={s.head} className="pc-pix-head">
    <div><span style={s.kicker}>COMPRA DIRETA · PIX</span><h2 id="pix-title">Finalize seu pedido</h2><p style={s.bookName}>{book.name}</p><p style={s.headText}>Preencha seus dados e gere o Pix com segurança.</p></div>
    <svg className="pc-pix-head-art" viewBox="0 0 180 108" aria-hidden="true"><defs><linearGradient id="pc-checkout-gold" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#f4dca5"/><stop offset="1" stopColor="#c18b45"/></linearGradient><linearGradient id="pc-checkout-plum" x1="0" y1="0" x2="1" y2="1"><stop stopColor="#4b2b55"/><stop offset="1" stopColor="#291632"/></linearGradient></defs><path d="M22 22h55c12 0 21 7 24 16v53c-5-8-13-12-24-12H22Z" fill="url(#pc-checkout-plum)"/><path d="M158 22h-36c-11 0-18 6-21 16v53c5-8 12-12 22-12h35Z" fill="#fff" stroke="#d7c7d8" strokeWidth="2"/><path d="M35 38h43M35 49h36M35 60h28" stroke="#efd79f" strokeWidth="3" strokeLinecap="round" opacity=".78"/><rect x="119" y="36" width="25" height="25" rx="4" fill="url(#pc-checkout-gold)"/><path d="m126 48 5 5 8-10" fill="none" stroke="#38203f" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/><path d="M113 70h34" stroke="#77527e" strokeWidth="3" strokeLinecap="round"/><circle cx="101" cy="91" r="4" fill="#d2a75f"/></svg>
  </div>
...
const s:Record<string,React.CSSProperties>={
 ...
 modal:{width:"min(680px,100%)",maxHeight:"calc(100dvh - 16px)",overflow:"auto",background:"#fff",border:"1px solid rgba(240,211,151,.28)",borderRadius:16,position:"relative",boxShadow:"0 24px 70px rgba(0,0,0,.34)",fontFamily:"Inter,system-ui,sans-serif",color:"#241d27"},
 ...
 brandBar:{minHeight:44,padding:"0 56px 0 20px",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#281630",color:"#fff",borderBottom:"1px solid rgba(240,211,151,.22)",fontSize:9,fontWeight:900,letterSpacing:".09em"},
 head:{position:"relative",minHeight:88,padding:"11px 190px 10px 20px",display:"flex",alignItems:"center",overflow:"hidden",backgroundColor:"#f1e9f3",backgroundImage:"linear-gradient(90deg,rgba(251,248,252,.99) 0%,rgba(241,233,243,.96) 70%,rgba(241,233,243,.78) 100%),url('/dorinha-hero-editorial-v3.png')",backgroundPosition:"center,right center",backgroundSize:"cover,260px auto",backgroundRepeat:"no-repeat",borderBottom:"1px solid #e8dfea"},kicker:{fontSize:9,fontWeight:900,letterSpacing:".14em",color:"#7a4e83"},bookName:{fontWeight:850,color:"#513d57",margin:"4px 0 1px",fontSize:12},headText:{color:"#756a78",fontSize:11,lineHeight:1.4},
 form:{display:"grid",gap:7,padding:"11px 20px 13px"},orderSummary:{display:"grid",gridTemplateColumns:"1fr auto",gap:12,alignItems:"center",padding:"9px 11px",border:"1px solid #e8e1e9",borderRadius:10,background:"#fcfbfc"},price:{textAlign:"right",display:"grid",gap:2},sectionLabel:{fontSize:9,fontWeight:900,letterSpacing:".13em",color:"#806e84",marginTop:1},
 fields:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px 10px"},two:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8},label:{display:"grid",gap:3,fontSize:10,fontWeight:800,color:"#493e4c"},input:{width:"100%",minHeight:36,border:"1px solid #dcd4de",borderRadius:8,padding:"0 10px",font:"inherit",fontSize:12,color:"#29202c",background:"#fff",outline:"none",boxSizing:"border-box",transition:".18s ease"},
 ...
 primary:{minHeight:39,border:0,borderRadius:9,background:"linear-gradient(135deg,#44294e,#2d1935)",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontWeight:900,cursor:"pointer",textDecoration:"none",fontSize:12,boxShadow:"0 8px 18px rgba(57,32,65,.18)"},disabled:{opacity:.55,cursor:"not-allowed",boxShadow:"none"},secondary:{minHeight:40,border:"1px solid #ddd5df",borderRadius:9,background:"#fff",color:"#45384b",display:"flex",alignItems:"center",justifyContent:"center",gap:7,fontWeight:800,cursor:"pointer",textDecoration:"none"},
 error:{padding:13,borderRadius:11,background:"#fff0f0",border:"1px solid #f3cccc",color:"#932c2c",fontSize:12,display:"grid",gap:4},helper:{color:"#7c727f",fontSize:9,lineHeight:1.4,textAlign:"center"},trustRow:{display:"flex",justifyContent:"center",flexWrap:"wrap",gap:13,color:"#5e6c63",fontSize:9,fontWeight:800},secureNote:{display:"flex",alignItems:"center",justifyContent:"center",gap:6,color:"#657168",fontSize:11},
 ...
 qrSection:{display:"grid",placeItems:"center",textAlign:"center",gap:8,padding:"18px 22px"},iconCircle:{width:44,height:44,borderRadius:"50%",display:"grid",placeItems:"center",background:"#f2ebf4",color:"#65456d"},qrFrame:{position:"relative",width:"min(204px,36vh)",height:"min(204px,36vh)",display:"grid",placeItems:"center",overflow:"hidden",background:"#fff",border:"1px solid #e7e0e8",borderRadius:14},qrLoading:{color:"#6f6273",fontSize:12,fontWeight:800},qrError:{position:"absolute",inset:0,padding:18,display:"grid",placeContent:"center",gap:10,background:"#fff7f7",color:"#8c3030",fontSize:12},bookQr:{position:"absolute",inset:6,width:"calc(100% - 12px)",height:"calc(100% - 12px)",objectFit:"contain",background:"white",transition:"opacity .2s ease"}
};
