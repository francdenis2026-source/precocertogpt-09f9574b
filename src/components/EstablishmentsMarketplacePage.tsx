...
export function EstablishmentsMarketplacePage(){
  ...
  useEffect(()=>{ void (async()=>{
    ...
    const now = Date.now();
    const merged = (establishments??[]).map((establishment:any)=>{
    ...
    }).filter(item=>showDemo||!item.demo).sort((a,b)=>Number(b.sponsored)-Number(a.sponsored)||Number(b.online||b.direct)-Number(a.online||a.direct)||Number(b.is_verified)-Number(a.is_verified)||a.name.localeCompare(b.name,"pt-BR"));
  ...
  const catalogUrl = (item:DirectoryItem)=>`/estabelecimento/${encodeURIComponent(item.slug||item.id)}`;
  ...
  return <div className="est-page">
    <header className="est-header">
      <div className="est-shell est-header__inner">
    ...
    <main>
      <section className="est-hero">
        <div className="est-hero__image" aria-hidden="true"/><div className="est-hero__overlay" aria-hidden="true"/>
        <div className="est-shell est-hero__inner">
          <div><span className="est-kicker"><MapPin size={15}/> comércio local de Feijó</span><h1>Encontre estabelecimentos e <span>explore cada catálogo.</span></h1><p>Veja mercados, farmácias e outros negócios locais, consulte produtos e preços e entre direto na vitrine de cada estabelecimento.</p><div className="est-hero__actions"><a className="est-btn est-btn--primary" href="#estabelecimentos"><Store size={18}/> Explorar lojas</a><a className="est-btn est-btn--ghost" href="/lojista">Cadastrar meu negócio <ChevronRight size={17}/></a></div></div>
          <aside className="est-stats" aria-label="Resumo dos estabelecimentos"><div className="est-stat"><strong>{items.length}</strong><span>estabelecimentos cadastrados</span></div><div className="est-stat"><strong>{purchasable}</strong><span>com compra disponível</span></div><div className="est-stat"><strong>{verified}</strong><span>cadastros verificados</span></div></aside>
        </div>
      </section>
      ...
      <section id="estabelecimentos" className="est-directory"><div className="est-shell">
        <div className="est-heading"><div><span>Diretório local</span><h2>Escolha onde quer pesquisar.</h2><p>Filtre por nome, bairro ou segmento e entre diretamente no catálogo do estabelecimento.</p></div><a href="/lojista">Cadastrar meu negócio <ChevronRight size={16}/></a></div>
        <div className="est-filters"><label className="est-search"><Search size={18} aria-hidden="true"/><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Buscar estabelecimento ou bairro" aria-label="Buscar estabelecimento ou bairro"/></label><select className="est-select" value={kind} onChange={event=>setKind(event.target.value)} aria-label="Filtrar por segmento"><option value="all">Todos os segmentos</option>{categories.map(category=><option key={category} value={category}>{labels[category]??category}</option>)}</select><button className={`est-filter${onlyPurchasable?" is-active":""}`} type="button" onClick={()=>setOnlyPurchasable(value=>!value)}><ShoppingBag size={16}/> Compra disponível</button></div>
        {loading?<div className="est-empty">Carregando estabelecimentos…</div>:<div className="est-grid">{filtered.map(item=>{
          const segment = item.merchant?.business_type||item.kind||"other";
          const author = item.slug==="dorinha-barroso-livros"||segment==="books_author";
          const logo = item.logo_url || getStoreLogoUrl(item.name);
          const target = catalogUrl(item);
          return <article key={item.id} className="est-card" role="link" tabIndex={0} aria-label={`Abrir catálogo de ${item.name}`} onClick={()=>navigate(target)} onKeyDown={event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();navigate(target)}}}>
            {item.sponsored&&<div className="est-ribbon">PATROCINADO</div>}
            <div className="est-card__head"><div className="est-logo" style={{background:item.brand_color||"#173d2a"}}>{logo?<img src={logo} alt={`Logo ${item.name}`} loading="lazy" onError={event=>{event.currentTarget.style.display="none"}}/>:<Store aria-hidden="true"/>}</div><div className="est-badges">{item.partner_tier==="premium"&&<span className="est-badge est-badge--premium"><Sparkles size={12}/> Premium</span>}{item.is_verified&&<span className="est-badge est-badge--verified"><BadgeCheck size={12}/> Verificado</span>}{(item.online||item.direct)&&<span className="est-badge est-badge--online"><ShoppingBag size={12}/> Compra disponível</span>}</div></div>
            <div className="est-card__body"><span className="est-type">{labels[segment]??"Comércio local"}</span><h3>{item.name}</h3><p>{item.short_description||"Explore o catálogo e os preços deste estabelecimento no PreçoCerto."}</p><div className="est-meta">{item.neighborhood&&<span><MapPin size={14}/>{item.neighborhood}</span>}{item.merchant?.delivery_enabled&&<span><Truck size={14}/>Entrega</span>}{item.merchant?.pickup_enabled&&<span><Building2 size={14}/>Retirada</span>}</div><div className="est-enter"><span>Entrar no estabelecimento</span><ChevronRight size={17}/></div></div>
            <div className="est-card__footer" onClick={event=>event.stopPropagation()}>{author?<a className="est-action--primary" href="/autora/dorinha-barroso"><BookOpen size={16}/> Espaço da autora</a>:item.online&&item.merchant?<a className="est-action--primary" href={`/loja/${item.merchant.id}`}><ShoppingBag size={16}/> Comprar online</a>:<a className="est-action--primary" href={target}><Store size={16}/> Ver catálogo</a>}<a className="est-action--secondary" href={target}>Explorar <ChevronRight size={15}/></a></div>
          </article>})}{!filtered.length&&<div className="est-empty">Nenhum estabelecimento encontrado com esses filtros.</div>}</div>}
      </div></section>
