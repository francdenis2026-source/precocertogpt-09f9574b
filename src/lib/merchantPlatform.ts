...
export type OrderStatus = "pending_review" | "pending_payment" | "paid" | "accepted" | "preparing" | "ready" | "out_for_delivery" | "delivered" | "cancelled";
export type PaymentStatus = "pending" | "approved" | "rejected" | "refunded" | "cancelled";
export type MerchantOrderItem = { id:string; order_id:string; product_id:string|null; merchant_product_id?:string|null; product_name:string; quantity:number; unit_price:number; total_price:number; image_url:string|null; variant_snapshot?:Record<string,unknown>|null; modifier_snapshot?:Array<Record<string,unknown>>; item_notes?:string|null; regulatory_snapshot?:Record<string,unknown> };
export type MerchantOrder = { id:string; order_number:string; merchant_id:string; customer_id:string|null; customer_name:string; customer_phone:string|null; customer_email:string|null; delivery_address:Record<string,string>|null; delivery_type:"delivery"|"pickup"; status:OrderStatus; payment_status:PaymentStatus; payment_provider:string|null; subtotal:number; delivery_fee:number; discount:number; platform_fee:number; total:number; notes:string|null; operation_stage?:string|null; created_at:string; updated_at:string; items?:MerchantOrderItem[] };
...
export type MarketplaceOrderItemInput = { merchant_product_id:string; quantity:number; variant_id?:string|null; modifier_option_ids?:string[]; notes?:string };
...
export async function loadMerchantSummary(merchantId:string,merchantName="Meu estabelecimento"):Promise<MerchantSummary>{ const orders=await loadMerchantOrders(merchantId,200),today=new Date().toISOString().slice(0,10),todayOrders=orders.filter(o=>o.created_at.slice(0,10)===today),completed=todayOrders.filter(o=>!["cancelled","pending_payment","pending_review"].includes(o.status)),todayGross=completed.reduce((s,o)=>s+o.total,0); let lowStock=0;if(supabase){const {count}=await supabase.from("merchant_products").select("id",{count:"exact",head:true}).eq("merchant_id",merchantId).eq("active",true).lte("stock_quantity",5);lowStock=count??0} return{merchantId,merchantName,todayGross,todayOrders:todayOrders.length,pendingOrders:orders.filter(o=>["pending_review","paid","accepted"].includes(o.status)).length,preparingOrders:orders.filter(o=>o.status==="preparing").length,deliveryOrders:orders.filter(o=>o.status==="out_for_delivery").length,averageTicket:completed.length?todayGross/completed.length:0,lowStock}; }
...
export async function createMarketplaceOrder(input:{merchantId:string;deliveryType:"delivery"|"pickup";deliveryZoneId?:string|null;deliveryAddress?:Record<string,string>|null;customerName:string;customerPhone?:string;customerEmail?:string;items:MarketplaceOrderItemInput[];notes?:string}){
...
export async function reviewPharmacyOrder(orderId:string,decision:"approve"|"reject",notes?:string){if(!supabase)return{data:null,error:"Supabase indisponível"};const {data,error}=await supabase.rpc("review_pharmacy_order",{_order_id:orderId,_decision:decision,_notes:notes??null});return{data,error:error?.message??null};}
