...
async function cpfAlias(cpf:string){const bytes=new TextEncoder().encode(`precocerto-owner:${cpf}`),hash=await crypto.subtle.digest("SHA-256",bytes);return `u-${Array.from(new Uint8Array(hash)).map(x=>x.toString(16).padStart(2,"0")).join("")}@login.precocerto.com.br`}
